...
**Purpose:** Pure functions that take mindMapNodes + mindMapLinks + project entities and derive the structured data Corkboard needs to render. No React, no hooks — just data transformation.

**Consumes:** `resolveNodeDisplay` from `./mindMapUtils`

**Produces:** Two exported functions: `getCorkboardData` and `moveSceneToAct`
...
- [ ] **Step 1:** Create `src/lib/corkboardFromMindMap.js` with the exact code above
- [ ] **Step 2:** Read the file to verify it was written correctly
- [ ] **Step 3:** Commit with message: `feat: add corkboardFromMindMap utility with getCorkboardData and moveSceneToAct`

## Verification

1. Confirm file exists at `src/lib/corkboardFromMindMap.js`
2. Confirm both functions are exported
3. Confirm imports are correct (resolveNodeDisplay from ./mindMapUtils)